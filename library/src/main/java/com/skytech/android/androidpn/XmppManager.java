...
public class XmppManager implements IXmppManager {
    ...
    public XmppManager(NotificationService notificationService) {
        ...
        apiKey= sharedPrefs.getString(Constants.API_KEY, "");
    ...
    private class ConnectTask implements Runnable {
        ...
        public void run() {
            ...
            if (!xmppManager.isConnected()) {
                ...
                connConfig.setSecurityMode(SecurityMode.disabled);//不使用TLS链路跟服务器进行通信
                //connConfig.setSecurityMode(SecurityMode.required);//如果发现smack客户端有出现No response from the server的exception，可尝试更改连接选项为disabled
            ...
            if (!xmppManager.isAuthenticated()) {
                ...
                try {
                    xmppManager.getConnection().login(
                            ...
                            xmppManager.getPassword(), SkyInitializer.getInstance().getPackageName());
                    ...
                    Log.e(LOGTAG, "Failed to activity_login to xmpp server. Caused by: "
                            + e.getMessage());
