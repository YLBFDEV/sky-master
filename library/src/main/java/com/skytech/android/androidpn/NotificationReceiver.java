...
public final class NotificationReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Constants.ACTION_SHOW_NOTIFICATION.equals(action)) {
            SharedPreferences sharedPrefs = context.getSharedPreferences(Constants.SHARED_PREFERENCE_NAME, Context.MODE_PRIVATE);
            String apiKey = sharedPrefs.getString(Constants.API_KEY, "");
            ...
            if (apiKey.equals(notificationApiKey)) {
                //处理本应用的推送消息
                Intent broadcast = new Intent();
                broadcast.setAction(Constants.ACTION_NOTIFICATION_BROADCAST);
                broadcast.setPackage(context.getPackageName());
                broadcast.putExtra(Constants.NOTIFICATION_URI, notificationUri);
