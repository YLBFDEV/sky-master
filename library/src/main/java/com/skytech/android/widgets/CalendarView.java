...
public class CalendarView extends View implements View.OnTouchListener {
    ...
    private void init() {
        ...
        showFirstDate = showLastDate = curDate;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        surface.height = (int) (getResources().getDisplayMetrics().heightPixels * 0.57f);
...
//		canvas.drawRect(surface.weekHeight, surface.width, surface.weekHeight, surface.width, surface.cellBgPaint);
        ...
        for (int i = 0; i < 42; i++) {
            ...
            if (todayIndex != -1 && i == todayIndex) {
                ...
                drawCellBg(canvas, i, surface.todayBgColor);
            }
            drawCellText(canvas, i, date[i] + "", color);
            int dotColor = getDotColor(i);
            if (dotColor != -1) {
                drawCellDot(canvas, i, dotColor);
            }
        }
        super.onDraw(canvas);
    }
    ...
    private void drawCellText(Canvas canvas, int index, String text, int color) {
        ...
        float cellY = surface.monthHeight + surface.weekHeight + (y - 1)
                * surface.cellHeight + surface.cellHeight * 0.6f;
    ...
    private void drawCellBg(Canvas canvas, int index, int color) {
        ...
        float radius = surface.cellWidth / 2 - 3;
        float cx = surface.cellWidth * (x - 1) + surface.borderWidth + surface.cellWidth / 2;
        float cy = surface.monthHeight + surface.weekHeight + (y - 1)
      ...
      /*  canvas.drawRect(left, top, left + surface.cellWidth
        ...
        canvas.drawCircle(cx, cy, radius, surface.cellBgPaint);// 绘制圆
    }

    private void drawCellDot(Canvas canvas, int index, int color) {
        int x = getXByIndex(index);
        int y = getYByIndex(index);
        surface.cellBgPaint.setColor(color);
        float radius = 4;
        float cx = surface.cellWidth * (x - 1) + surface.borderWidth + surface.cellWidth / 2;
        float cy = surface.monthHeight + surface.weekHeight + (y - 1)
                * surface.cellHeight + surface.cellHeight * 0.76f;
        canvas.drawCircle(cx, cy, radius, surface.cellBgPaint);// 绘制圆点
    }
    ...
    private Map<String, Integer> dotColors;

    public void setDotColor(Map<String, Integer> dotColors) {
        this.dotColors = dotColors;
        postInvalidate();
    }

    private int getDotColor(int i) {
        calendar.setTime(showFirstDate);
        calendar.add(Calendar.DAY_OF_YEAR, i);
        String dataStr = DateUtils.formatDate(calendar.getTime());
        if (null == dotColors || !dotColors.containsKey(dataStr)) {
            return -1;
        } else {
            return dotColors.get(dataStr);
        }
    }
...
//				}
    ...
    private class Surface {
        ...
        public int bgColor = Color.parseColor("#F7F9FE");
        ...
        public int todayBgColor = Color.RED;
        ...
        public void init() {
            ...
            boxPath.moveTo(0, monthHeight + weekHeight);//光标移动指定的坐标点
            boxPath.rLineTo(width, 0);//在moveTo方法中指定的直线起点与参数中指定的直线终点之间绘制一条直线
            for (int i = 1; i < 7; i++) {
