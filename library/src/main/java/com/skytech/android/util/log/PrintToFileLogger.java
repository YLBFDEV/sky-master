...
import com.skytech.android.util.cache.ExternalOverFroyoUtils;
import com.skytech.android.util.cache.ExternalUnderFroyoUtils;
...
public class PrintToFileLogger implements ILogger {
    ...
    private static String BASE_FILENAME = "skytech.log";
    private File logDir;
    ...
    public void open() {
        if (AndroidVersionCheckUtils.hasFroyo()) {
            logDir = ExternalOverFroyoUtils.getDiskCacheDir(SkyInitializer
                    .getInstance().getContext(), LOG_DIR);
        } else {
            logDir = ExternalUnderFroyoUtils.getDiskCacheDir(SkyInitializer
                    .getInstance().getContext(), LOG_DIR);
        }
        if (!logDir.exists()) {
            logDir.mkdirs();
        ...
        switch (priority) {
            case VERBOSE:
                printMessage = "[V]|"
                        + tag
                        + "|"
                        + SkyInitializer.getInstance().getContext()
