...
public class FileCache {
                ...
                if (mCacheParams.diskCacheEnabled && diskCacheDir != null) {
                    ...
                    if (usableSpace > mCacheParams.diskCacheSize) {
                        try {
                            ...
                            ArkLogger.e(LOG_TAG, "initDiskCache - " + e);
                        }
                    }
                }
            }
        ...
        synchronized (mDiskCacheLock) {
            ...
            if (mDiskLruCache != null) {
                ...
                try {
                    ...
                    ArkLogger.e(LOG_TAG, "addBufferToCache - " + e);
                } catch (Exception e) {
                    ArkLogger.e(LOG_TAG, "addBufferToCache - " + e);
                } finally {
                    try {
    ...
    public byte[] getBufferFromMemCache(String data) {
        ...
        try {
            ...
            if (memValue == null) {
                memValue = getBufferFromDiskCache(data);
                if (memValue !=
                        null && mMemoryCache != null && mMemoryCache.get(data) == null) {
                    mMemoryCache.put(data, memValue);// 添加数据到内存缓存
                }
            }
        } catch (Exception e) {
            // TODO: handle exception
            e.printStackTrace();
            ArkLogger.e(LOG_TAG, "获取缓存数据失败！");
        }
        return memValue;
    }
        ...
        synchronized (mDiskCacheLock) {
            ...
            if (mDiskLruCache != null) {
                ...
                try {
                    ...
                    ArkLogger.e(LOG_TAG, "getBufferFromDiskCache - "
                            + e);
                } catch (Exception e) {
        ...
        synchronized (mDiskCacheLock) {
            ...
            if (mDiskLruCache != null && !mDiskLruCache.isClosed()) {
                try {
                    ...
                    ArkLogger.e(LOG_TAG, "clearCache - " + e);
                }
                mDiskLruCache = null;
                initDiskCache();
            }
     ...
     * @version V1.0
     * @Title CacheParams
     * @Package org.zhangyk.util.cache
     * @Description 缓存的参数类
     */
