...
public class TiledBitmapCanvas implements CanvasLite {
    ...
    public static final int MAX_VERSIONS = 15;//最大撤销次数
    ...
    private class Tile {
        ...
        ArrayList<Version> removedVersions;
        private int oldTop;
        ...
        public Tile(int x, int y, int version) {
            ...
            removedVersions = new ArrayList<Version>(MAX_VERSIONS);
            bottom = version;
        ...
        private Version createVersion(int version) {
            ...
            if (null != mSlateHandler) {
                mSlateHandler.onRedoEnabledChanged(false);
                mSlateHandler.onUndoEnabledChanged(true);
            }
            return v;
        }

        private int findRemovedVersion(int version) {
            for (int i = 0; i < removedVersions.size(); i++) {
                final Version v = removedVersions.get(i);
        ...
        public void revert(int toVersion) {
            ...
            if (i < 0) {
                ...
                if (findRemovedVersion(toVersion) != -1 && toVersion >= oldTop) {
                    versions.add(0, removedVersions.get(0));
                    removedVersions.remove(0);
                    final int oldTop = top;
                    if (DEBUG) {
                        Log.v(TAG, String.format(" redo tile [%2d,%2d]: revert(%d) old top %d, %s",
                                x, y, toVersion, oldTop, debugVersions()));
                    }
                    top = versions.get(0).version;
                }
            } else if (i > 0) {
                for (int j = 0; j < i; j++) {
                    removedVersions.add(0, versions.get(j));
                }
                versions.subList(0, i).clear();
                oldTop = top;
                if (DEBUG) {
                    Log.v(TAG, String.format(" undo tile [%2d,%2d]: revert(%d) old top %d, %s",
                            x, y, toVersion, oldTop, debugVersions()));
                }
                top = versions.get(0).version;
            }
        ...
        public int getVersionsSize() {
           /* if (DEBUG) {
                Log.v(TAG, String.format(" tile [%2d,%2d]: versions size:%d", x, y, versions.size()));
            }*/
            return versions.size();
        }

        public int getRemovedVersionsSize() {
            return removedVersions.size();
        }
    }
    ...
    public void setSlateHandler(SlateView.SlateHandler slateHandler) {
        this.mSlateHandler = slateHandler;
    }

    private SlateView.SlateHandler mSlateHandler;
    ...
    public void step(int delta) {
        ...
        for (int i = 0; i < mTiles.length; i++) {
            ...
            if (tile.getVersionsSize() > 1) undoEnabled = true;
            if (tile.getRemovedVersionsSize() > 0) redoEnabled = true;
        }
        mNewVersion = newTop + 1;
        mVersionInUse = false;
        if (null != mSlateHandler) {
            mSlateHandler.onUndoEnabledChanged(mBottomVersion == newTop ? false : undoEnabled);
            mSlateHandler.onRedoEnabledChanged(redoEnabled);
        }
    }
}
