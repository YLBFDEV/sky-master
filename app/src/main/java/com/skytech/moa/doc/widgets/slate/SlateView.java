...
//                break;
    ...
    public SlateView(Context c, AttributeSet as) {
        super(c, as);
        init();
    }

    public SlateView(Context c) {
        this(c, null);
    }
...
//        mCircleBitsFrame = new Rect(0, 0, mCircleBits.getWidth(), mCircleBits.getHeight());
    ...
    public void setSlateHandler(SlateHandler slateHandler) {
        if (null == mTiledCanvas) return;
        this.mTiledCanvas.setSlateHandler(slateHandler);
    }
    ...
    public void redo() {
        if (mTiledCanvas == null) {
            Log.v(TAG, "redo before mTiledCanvas inited");
        }
        mTiledCanvas.step(1);
        invalidate();
    }
    ...
    public void initCanvas() {
        ...
        canvas.drawRect(0, 0, bitmap.getWidth(), bitmap.getHeight(), p);// 长方形
        setBitmap(bitmap);
    }
...
/*        Bitmap bitmap = Bitmap.createBitmap(getWidth(), getHeight(), Bitmap.Config.ARGB_8888);
        ...
        p.setStyle(Paint.Style.FILL);//设置填满
        canvas.drawRect(0, 0, bitmap.getWidth(), bitmap.getHeight(), p);// 长方形
        x = (bitmap.getWidth() - rawBitmap.getWidth())/2;
        y = (bitmap.getHeight() - rawBitmap.getHeight())/2;
        canvas.drawBitmap(rawBitmap, x, y, p);// 长方形
        setBitmap(bitmap);*/
        setBitmap(rawBitmap);
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw,
        ...
        if (mTiledCanvas == null) {
            throw new RuntimeException("onSizeChanged: Unable to allocate home_pad buffer (" + w + "x" + h + ")");
        }
